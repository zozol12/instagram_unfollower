...
class InstagramUnfollower:
    ...
    def __init__(self, username, password, unfollowing_speed):
        ...
        self.driver = webdriver.Firefox(
            executable_path=r'C:\Program Files\geckodriver.exe')  # specifies the driver which you want to use)
    ...
    def find_username(
            self):  # finds the username in the case of the program user is entered an e-mail address instead of the
        # username as a login info
        driver = self.driver
        self._username = driver.find_element_by_xpath("//a[@class='gmFkV']").text

    def find_followings(self, driver, buttons):  # this functions finds the accounts that we follow
        ...
        time.sleep(2)
        self.follower_number = driver.find_element_by_xpath(
